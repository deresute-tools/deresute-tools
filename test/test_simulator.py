...
# sim.simulate(appeals=277043, time_offset=120, auto=True) == 561672
#
# unit = Unit.from_list([200946, 200058, 100076, 100396, 300530, 200294], custom_pots=(0, 0, 0, 0, 10))
# live = Live()
...
# live.set_unit(unit)
# sim = Simulator(live, special_offset=0.075)
# sim.simulate(auto=True, appeals=277043, time_offset=110) == 570342
#
# unit = Unit.from_list([200946, 200058, 100076, 100396, 300530, 200294], custom_pots=(0, 0, 0, 0, 10))
# live = Live()
...
# live.set_unit(unit)
# sim = Simulator(live, special_offset=0.075)
# sim.simulate(auto=True, appeals=277043, time_offset=400) == 629870
#
# unit = Unit.from_list([200946, 200058, 100076, 100396, 300530, 200294], custom_pots=(0, 0, 0, 0, 10))
# live = Live()
...
# live.set_unit(unit)
# sim = Simulator(live, special_offset=0.075)
# sim.simulate(auto=True, appeals=277043, time_offset=113) == 654967
#
...
# live.set_unit(gu)
# sim = Simulator(live, special_offset=0.075)
# sim.simulate(auto=True, appeals=444572, time_offset=120) == 991859
#
# unitA = Unit.from_list([200946, 200496, 100396, 300572, 200058], custom_pots=(0, 0, 0, 0, 10))
...
# live.set_unit(gu)
# sim = Simulator(live, special_offset=0.075)
# sim.simulate(auto=True, appeals=451228, time_offset=218, mirror=True) == 936614
...
# assert res.abuse_score - res.perfect_score == 54786
#
...
# assert res.abuse_score - res.perfect_score == 76303
#
...
# sim = Simulator(live)
# res = sim.simulate(support=113290, perfect_play=True, abuse=True)
# assert res.abuse_score == 7219260
#
...
# assert res.abuse_score - res.perfect_score == 47441
#
...
# live.set_unit(gu)
# sim = Simulator(live)
# assert sim.simulate(times=10, perfect_play=True, abuse=True).perfect_score == 8939389
...
assert sim.simulate(perfect_play=True).perfect_score == 5302939
